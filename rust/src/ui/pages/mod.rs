//! The three tab pages plus a placeholder for pages not ported yet.
...
pub fn activate_item(ctx: &Rc<UiContext>, item: &MediaItem, pool: &[MediaItem]) {
    match item.kind {
        ...
        ItemKind::Album => ctx.nav.go(NavRequest::Album { id: item.id.clone(), title: item.title.clone(), thumb: item.thumb.clone() }),
