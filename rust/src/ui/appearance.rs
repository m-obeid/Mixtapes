...
//!
//! Three display-wide CSS providers, kept separate so toggling one feature
//! leaves the others alone. They sit one step above the user's gtk.css.
...
pub struct Appearance {
    ...
    derived_css: gtk::CssProvider,
    /// True while bg_css carries the override sheet, with or without a picture.
    bg_loaded: Cell<bool>,
...
impl Appearance {
    pub fn new(window: &adw::ApplicationWindow, ctx: &Rc<UiContext>) -> Rc<Self> {
        let this = Rc::new(Self {
            ...
            accent_css: gtk::CssProvider::new(),
            derived_css: gtk::CssProvider::new(),
            bg_loaded: Cell::new(false),
        ...
        if let Some(display) = gdk::Display::default() {
            // The derived provider goes last so it wins the tie against the accent one.
            for provider in [&this.bg_css, &this.accent_css, &this.derived_css] {
    ...
    fn activate_cover_bg(self: &Rc<Self>, url: &str) {
        self.window.add_css_class("cover-bg-active");
        if !self.bg_loaded.replace(true) {
            self.bg_css.load_from_string(BLUR_OVERRIDE_CSS);
        }
        self.update_blurred_background(url);
    }

    fn deactivate_cover_bg(&self) {
        self.window.remove_css_class("cover-bg-active");
        self.blur_backdrop.set(None);
        self.blur_request.set(self.blur_request.get() + 1);
        self.bg_loaded.set(false);
        self.bg_css.load_from_string("");
    }
    ...
    fn set_blurred_background_css(&self, path: &std::path::Path) {
        ...
        self.bg_loaded.set(true);
        self.bg_css.load_from_string(&format!("{BLUR_OVERRIDE_CSS}{rule}"));
    }
    ...
    /// Queue a recompute for the next idle. GTK notifies a scheme change before
    /// it swaps the stylesheet, so deriving inside the notify reads the theme
    /// being left. Coalesced, since several callers fire together.
    ...
    fn derive_colors(&self) {
        ...
        self.derived_css.load_from_string(&format!(
