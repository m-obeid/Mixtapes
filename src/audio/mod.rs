...
fn leave_pictures_undecoded(playbin: &gst::Element) {
    playbin.connect("element-setup", false, |values| {
        ...
        if element.factory().is_none_or(|f| f.name() != "uridecodebin") {
            return None;
        }
        element.connect("autoplug-select", false, |values| {
            let factory = values[3].get::<gst::ElementFactory>().ok()?;
            let result = glib::EnumClass::with_type(glib::Type::from_name("GstAutoplugSelectResult")?)?;
            result.to_value_by_nick(if decodes_pictures(factory.metadata(gst::ELEMENT_METADATA_KLASS).unwrap_or_default()) { "expose" } else { "try" })
        });
        None
    });
}
...
mod tests {
    ...
    fn only_picture_and_video_decoders_are_left_out() {
        ...
        assert!(decodes_pictures("Codec/Decoder/Image"), "jpegdec, pngdec");
        assert!(!decodes_pictures("Codec/Decoder/Audio"), "opusdec");
        assert!(!decodes_pictures("Codec/Demuxer"), "matroskademux, qtdemux");
    ...
    fn a_song_with_cover_art_plays_without_a_picture_decoder() {
        gst::init().unwrap();
        let music = glib::user_special_dir(glib::UserDirectory::Music).unwrap().join("Mixtapes");
        fn first_audio(dir: &std::path::Path) -> Option<std::path::PathBuf> {
            ...
            entries.sort();
            entries.iter().find(|p| p.extension().is_some_and(|x| x == "mp3" || x == "opus" || x == "m4a")).cloned().or_else(|| entries.iter().filter(|p| p.is_dir() && !p.ends_with(".mixtapes")).find_map(|d| first_audio(d)))
        }
        // MIXTAPES_TEST_MEDIA points it at any file, such as one with a video track.
        let file = std::env::var_os("MIXTAPES_TEST_MEDIA").map(std::path::PathBuf::from).or_else(|| first_audio(&music)).expect("a downloaded song");
        println!("playing {}", file.display());

        let playbin = gst::ElementFactory::make("playbin").build().unwrap();
        let flags = playbin.property_value("flags");
        let class = glib::FlagsClass::with_type(flags.type_()).unwrap();
        playbin.set_property_from_value("flags", &class.builder_with_value(flags).unwrap().unset_by_nick("video").unset_by_nick("text").build().unwrap());
        playbin.set_property("audio-sink", gst::ElementFactory::make("fakesink").build().unwrap());
        leave_pictures_undecoded(&playbin);
        let created = Arc::new(Mutex::new(Vec::<String>::new()));
        {
            let created = created.clone();
            playbin.connect("element-setup", false, move |values| {
                if let Some(factory) = values[1].get::<gst::Element>().ok().and_then(|e| e.factory()) {
                    created.lock().unwrap().push(format!("{} [{}]", factory.name(), factory.metadata(gst::ELEMENT_METADATA_KLASS).unwrap_or_default()));
                }
                None
            });
        }
        playbin.set_property("uri", glib::filename_to_uri(&file, None).unwrap().as_str());
        playbin.set_state(gst::State::Playing).unwrap();
        let bus = playbin.bus().unwrap();
        let mut playing = false;
        let deadline = std::time::Instant::now() + Duration::from_secs(8);
        while std::time::Instant::now() < deadline && !playing {
            if let Some(message) = bus.timed_pop(gst::ClockTime::from_mseconds(200)) {
                match message.view() {
                    gst::MessageView::Error(err) => panic!("pipeline error: {} {:?}", err.error(), err.debug()),
                    gst::MessageView::AsyncDone(_) => playing = true,
                    _ => {}
                }
            }
        }
        let elements = created.lock().unwrap().clone();
        playbin.set_state(gst::State::Null).unwrap();
        println!("elements: {elements:#?}");
        assert!(playing, "the song reached playing");
        assert!(!elements.iter().any(|e| decodes_pictures(e)), "no picture decoder: {elements:?}");
    }
